...
class GameController extends GetxController {
  ...
  Player player = const Player(id: "1", name: "Josco", position: 1, powers: 1);
  ...
  void startTimer() {
    _timer = Timer.periodic(const Duration(seconds: 1), (timer) {
      if (seconds > 0) {
        ...
        stop();
        showResultDialog(isWin: false, level: userLevel.value);
      }
    });
  }

  void showResultDialog({bool isWin = false, required int level}) {
    final levelDescription = getLevelDescription(level);
    Get.dialog(
      Dialog(
